...
def plot_bilayer():
    ...
    for i in range(bilayer_harmpairs.shape[0]):
        ...
        atom_2_crds = np.add(atom_1_crds, pbc_vector(atom_1_crds, atom_2_crds, scaled_dimensions))
...
def nets_to_bonding_pairs(nets: np.ndarray) -> np.ndarray:
    pairs = np.array([[node, bonded_node] for node in range(len(nets)) for bonded_node in nets[node] if bonded_node > node])
    return pairs

def nets_to_angle_triples(nets: np.ndarray) -> np.ndarray:
    angles = np.array([[nets[i][j], i, nets[i][(j+1)%len(nets[i])]] for i in range(len(nets)) for j in range(len(nets[i]))])
    return angles
...
def get_lammps_atoms_array_molecular(coords: np.array, molecule_ids: np.array = None, atom_types: list = None) -> list:
    ...
    if len(molecule_ids) != coords.shape[0] or len(atom_types) != coords.shape[0]:
        raise ValueError("Length of molecule_ids and atom_types must match length of coords.")
    atoms_array = []
    for i, (coordinate, molecule_id, atom_type) in enumerate(zip(coords, molecule_ids, atom_types)):
        atoms_array.append([i + 1, molecule_id, atom_type, coordinate[0], coordinate[1], coordinate[2]])
    return atoms_array
    ...
    with open(path, 'w') as data_file:
        ...
        for parameter, value in parameters.items():
            if parameter not in LAMMPS_DATA_DEFAULTS:
                raise ValueError(f"Parameter {parameter} is not a valid LAMMPS data parameter.")
            if value != LAMMPS_DATA_DEFAULTS[parameter]:
                data_file.write(f'{parameter} {value}\n')
        write_section("Atom Type Lables", dict_to_2d_array(atom_lables), data_file)
...
def pbc_vector(vector1: np.ndarray, vector2: np.ndarray, dimensions: np.ndarray) -> np.ndarray:
    ...
    difference_vector = (difference_vector + dimension_ranges / 2) % dimension_ranges - dimension_ranges / 2
...
def check_out_of_bounds(coords: np.ndarray, dimensions: np.ndarray) -> None:
    ...
    print("Dimensions: xlo: {:<10} xhi: {:<10}\n           ylo: {:<10} yhi: {:<10}").format(*dimensions.flatten())
...
def make_crds_marks_bilayer(path: Path, intercept, triangle_raft: bool, bilayer: bool, common_files_path: Path):
    ...
    dimensions = np.genfromtxt(path.joinpath("testA_a_aux.dat"), skip_header=3) # NetMC Aux dimensions are [[xhi, yhi], [xlo, ylo]]
    dimensions = dimensions.transpose()[:, ::-1] # Whereas LAMMPS dimensions are [[xlo, xhi], [ylo, yhi]]
    ...
    if triangle_raft:
        ...
        dimension_ranges = triangle_raft_dimensions[:, 1] - triangle_raft_dimensions[:, 0]
        
        triangle_raft_si_crds = node_crds * SI_SI_DISTANCE_BOHR * AREA_SCALING
        
        dict_sio = {i: [] for i in range(int(num_nodes_a * 3 / 2), int(num_nodes_a * 5 / 2))}

        tringle_raft_num_nodes = num_nodes_a
        for index, net in enumerate(nets):
            si_atom = index + 1 # LAMMPS atom ids start at 1, not 0
            for connected_si_atom in net:
                vector_between_si_atoms = pbc_vector(tringle_raft_si_crds[index], tringle_raft_si_crds[connected_si_atom - 1], triangle_raft_dimensions)
                ...
                midpoint = (tringle_raft_si_crds[index] + vector_between_si_atoms / 2 + dimension_ranges)
                ...
                triangle_raft_bonding_pairs = np.vstack((triangle_raft_bonding_pairs, np.array([si_atom, oxygen_atom_id])))
                triangle_raft_
        ...
        for pair_index, (node_1, node_2) in enumerate(monolayer_bonding_pairs):
            ...
            distance_vector = pbc_vector(atom_coordinates_1, atom_coordinates_2, triangle_raft_dimensions)
            normalized_vector = distance_vector / np.linalg.norm(distance_vector)

            dot_product_grades = np.abs(np.dot(normalized_vector, DISPLACEMENT_VECTORS_NORM.T))
            selected_vector_index = np.argmin(dot_product_grades)

            unperturbed_oxygen_coordinates = atom_coordinates_1 + distance_vector / 2
            oxygen_coordinates = unperturbed_oxygen_coordinates + (DISPLACEMENT_VECTORS_FACTORED[selected_vector_index] if dot_product_grades[selected_vector_index] < 0.1 else 0)
            oxygen_coordinates = (oxygen_coordinates + dimension_ranges) % dimension_ranges

            triangle_raft_o_crds = np.vstack((triangle_raft_o_crds, oxygen_coordinates)) if pair_index else oxygen_coordinates
            pair = np.array([[pair_index, node_1 + num_nodes_a * 1.5], [pair_index, node_2 + num_nodes_a * 1.5]])
            triangle_raft_bonding_pairs = np.vstack((triangle_raft_bonding_pairs, pair)) if pair_index else pair

            dict_sio.setdefault(int(node_1 + num_nodes_a * 1.5), []).append(pair_index)
            dict_sio.setdefault(int(node_2 + num_nodes_a * 1.5), []).append(pair_index)

        harmpairs_list = []
        for i in range(int(num_nodes_a * 1.5), int(num_nodes_a * 2.5)):
            pairs = [dict_sio[str(i)][j:k+1] for j in range(2) for k in range(j + 1, 3)]
        ...
        with open(path.joinpath("Si2O3.data"), 'w') as f:
            ...
            f.write('0.00000 {:<5} xlo xhi\n'.format(triangle_raft_dimensions[0]))
            f.write('0.00000 {:<5} ylo yhi\n'.format(triangle_raft_dimensions[1]))
            f.write('\n')
            f.write('# Pair Coeffs\n')
            f.write('#\n')
            f.write('# 1  O\n')
    ...
    if bilayer:
        ...
        for i in range(triangle_raft_si_crds.shape[0]):
            if i == 0:
                bilayer_o_crds = np.asarray([triangle_raft_si_crds[i, 0], triangle_raft_si_crds[i, 1], 5 + SI_O_LENGTH_BOHR])
            else:
                bilayer_o_crds = np.vstack((bilayer_o_crds, np.asarray([triangle_raft_si_crds[i, 0], triangle_raft_si_crds[i, 1], 5 + SI_O_LENGTH_BOHR])))
        # O eq
        for i in range(triangle_raft_o_crds.shape[0]):
            bilayer_o_crds = np.vstack((bilayer_o_crds, np.asarray([triangle_raft_o_crds[i, 0], triangle_raft_o_crds[i, 1], 5 - H_BOHR])))
            bilayer_o_crds = np.vstack((bilayer_o_crds, np.asarray([triangle_raft_o_crds[i, 0], triangle_raft_o_crds[i, 1], 5 + H_BOHR + 2 * SI_O_LENGTH_BOHR])))
        ...
        for i in range(triangle_raft_si_crds.shape[0]):
            if i == 0:
                bilayer_harmpairs = np.asarray([[i, 4 * num_nodes_a + 2 * i],  # 3200
                                                [i, 4 * num_nodes_a + 1 + 2 * i],  # 3201
                                                [i, map_index_to_bilayer(dict_sio['{:}'.format(int(3 * num_nodes_a / 2 + i))][0])[0]],
                                                [i, map_index_to_bilayer(dict_sio['{:}'.format(int(3 * num_nodes_a / 2 + i))][0])[1]],
                                                [i, map_index_to_bilayer(dict_sio['{:}'.format(int(3 * num_nodes_a / 2 + i))][1])[0]],
                                                [i, map_index_to_bilayer(dict_sio['{:}'.format(int(3 * num_nodes_a / 2 + i))][1])[1]],
                                                [i, map_index_to_bilayer(dict_sio['{:}'.format(int(3 * num_nodes_a / 2 + i))][2])[0]],
        ...
        for vals in dict_sio.keys():
            dict_sio2['{:}'.format(int(vals) - 3 * num_nodes_a / 2 + 4 * num_nodes_a)] = [map_index_to_bilayer(dict_sio["{:}".format(vals)][i]) for i in range(3)]
        ...
        with open(path.joinpath("PARM_SiO2.lammps"), 'w') as output_file:
            ...
            output_file.write('pair_coeff * * 0.1 {:} {:}\n'.format(O_O_DISTANCE_BOHR * intercept / 2**(1 / 6), O_O_DISTANCE_BOHR * intercept))
            output_file.write('pair_modify shift yes\n'.format())
            output_file.write('special_bonds lj 0.0 1.0 1.0\n'.format())
