...
def wrap_coord(coord: np.ndarray, dimensions: np.ndarray) -> np.ndarray:
    """
    Wrap a coordinate into the periodic boundary.
    """
    if len(coord) != len(dimensions):
        raise ValueError("Coordinate and dimensions must have the same number of dimensions.")
    dimension_ranges = dimensions[1] - dimensions[0]
    wrapped_coord = (coord - dimensions[0]) % dimension_ranges + dimensions[0]
